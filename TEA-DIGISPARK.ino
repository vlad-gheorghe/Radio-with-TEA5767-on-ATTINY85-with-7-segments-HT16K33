...
void setup()
{
 ...
 /// disp.set(2); // Устанавливаем яркость дисплея на максимум
  ...
  TinyWireM.begin();   // Инициализация двухпроводной шины I2C.
...
void setFrequency(double fmhz)  // Функция загрузки частоты настройки в модуль RDA5807.
{
  ...
  TinyWireM.beginTransmission(0x60);          // Адрес чипа RDA5807M
  TinyWireM.write(frequencyH);                // Старший байт.
  TinyWireM.write(frequencyL);                // Младший байт.
  TinyWireM.write(0xB8);                      // 1011 1000     =Стерео
  TinyWireM.write(0x10);                      // 0001 0000
  TinyWireM.endTransmission();                // формируем I2C-Stop.
}
